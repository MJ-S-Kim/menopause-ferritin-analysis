...
linear_spline <- function(data,vars,location){
  
  #This function is for piecewise regression with linear spline with menopause data.
  
  #data: analysis dataset
  #vars: response variables that want to analyse by piecewise model
  #location: location where results will be saved
  
  result <- NA
  for (var in vars){
    variable <- var
    #index <- which(colnames(df_iron) == var)
