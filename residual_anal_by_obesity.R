...
#cholesterol
res_tchol1 <- cbind("Total cholesterol", anova_result(df_model_baseline_obesity1$tchol, group, data = df_model_baseline_obesity1))
#ldl
res_ldl1 <- cbind("LDL", anova_result(df_model_baseline_obesity1$ldl, group, data = df_model_baseline_obesity1))
#sensitivity analysis for med usage
adjusted_ldl1 <- df_model_baseline_obesity1 %>% 
  mutate(adjusted_ldl = case_when(med_hyperlipidemia == 1 ~ ldl/0.7,
                                  med_hyperlipidemia == 0 ~ ldl))
res_adjusted_ldl1 <- cbind("adjusted LDL", anova_result(adjusted_ldl1$adjusted_ldl, group, data = adjusted_ldl1))
#hdl
res_hdl1 <- cbind("HDL", anova_result(df_model_baseline_obesity1$hdl, group, data = df_model_baseline_obesity1))
#tg
res_tg1 <- cbind("TG", anova_result(df_model_baseline_obesity1$tg, group, data = df_model_baseline_obesity1))
...
#cholesterol
res_tchol0 <- cbind("Total cholesterol", anova_result(df_model_baseline_obesity0$tchol, group, data = df_model_baseline_obesity0))
#ldl
res_ldl0 <- cbind("LDL", anova_result(df_model_baseline_obesity0$ldl, group, data = df_model_baseline_obesity0))
#sensitivity analysis for med usage
adjusted_ldl0 <- df_model_baseline_obesity0 %>% 
  mutate(adjusted_ldl = case_when(med_hyperlipidemia == 1 ~ ldl/0.7,
                                  med_hyperlipidemia == 0 ~ ldl))
res_adjusted_ldl0 <- cbind("adjusted LDL", anova_result(adjusted_ldl0$adjusted_ldl, group, data = adjusted_ldl0))
#hdl
#hdl
res_hdl0 <- cbind("HDL", anova_result(df_model_baseline_obesity0$hdl, group, data = df_model_baseline_obesity0))
#tg
res_tg0 <- cbind("TG", anova_result(df_model_baseline_obesity0$tg, group, data = df_model_baseline_obesity0))
...
res_list1 <- list(res_hypertension1, res_diabete1, res_gluc1, res_hba1c1, res_hyperlipidemia1, res_tchol1, res_ldl1,res_adjusted_ldl1,
                  res_hdl1, res_tg1, res_fatty_liver1)


res_list0 <- list(res_hypertension0, res_diabete0, res_gluc0, res_hba1c0, res_hyperlipidemia0, res_tchol0, res_ldl0,res_adjusted_ldl0,
...
res_hyperlipidemia1 <- cbind("Hyperlipidemia", row.names(tb_hyperlipidemia1), chisq_test_count_percent(tb_hyperlipidemia1))
#cholesterol
res_tchol1 <- cbind("Total cholesterol", anova_result(df_y5_obesity1$tchol, group, data = df_y5_obesity1))
#ldl
res_ldl1 <- cbind("LDL", anova_result(df_y5_obesity1$ldl, group, data = df_y5_obesity1))
#sensitivity analysis for med usage
adjusted_ldl1 <- df_y5_obesity1 %>% 
  mutate(adjusted_ldl = case_when(med_hyperlipidemia == 1 ~ ldl/0.7,
                                  med_hyperlipidemia == 0 ~ ldl))
res_adjusted_ldl1 <- cbind("adjusted LDL", anova_result(adjusted_ldl1$adjusted_ldl, group, data = adjusted_ldl1))
#hdl
res_hdl1 <- cbind("HDL", anova_result(df_y5_obesity1$hdl, group, data = df_y5_obesity1))
#tg
res_tg1 <- cbind("TG", anova_result(df_y5_obesity1$tg, group, data = df_y5_obesity1))

## obesity == 0
tb_hyperlipidemia0 <- table(df_y5_obesity0$hyperlipidemia, df_y5_obesity0$group)
res_hyperlipidemia0 <- cbind("Hyperlipidemia", row.names(tb_hyperlipidemia0), chisq_test_count_percent(tb_hyperlipidemia0))
#cholesterol
res_tchol0 <- cbind("Total cholesterol", anova_result(df_y5_obesity0$tchol, group, data = df_y5_obesity0))
#ldl
res_ldl0 <- cbind("LDL", anova_result(df_y5_obesity0$ldl, group, data = df_y5_obesity0))
#sensitivity analysis for med usage
adjusted_ldl0 <- df_y5_obesity0 %>% 
  mutate(adjusted_ldl = case_when(med_hyperlipidemia == 1 ~ ldl/0.7,
                                  med_hyperlipidemia == 0 ~ ldl))
res_adjusted_ldl0 <- cbind("adjusted LDL", anova_result(adjusted_ldl0$adjusted_ldl, group, data = adjusted_ldl0))
#hdl
res_hdl0 <- cbind("HDL", anova_result(df_y5_obesity0$hdl, group, data = df_y5_obesity0))
#tg
res_tg0 <- cbind("TG", anova_result(df_y5_obesity0$tg, group, data = df_y5_obesity0))
